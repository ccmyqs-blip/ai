...
async function requestSubscriptionGateway(path, token) {
  try {
    const resp = await fetch(`${SUBSCRIPTION_GATEWAY_BASE}${path}`, {
      method: "POST",
      headers: {
    ...
    return { ok: resp.ok, status: resp.status, body: parsed, path };
  } catch (error) {
    return {
      ok: false,
      status: 0,
